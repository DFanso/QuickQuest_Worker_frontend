    ...
    <div className="w-3/5 p-4 bg-white rounded-lg shadow-md my-4" style={{ boxShadow: '0px 0px 4px 0px rgba(0, 0, 0, 0.25)', borderRadius: '10px', background: '#FFF' }}>
      <div className="flex flex-wrap justify-between items-center my-4">
        ...
        {services.map((service) => (
          <div key={service._id} className="flex items-center mb-4 sm:mb-0 sm:ml-4">
            <button
              onClick={() => handleCreateOfferClick(service)}
              className="flex flex-row items-center justify-center px-4 py-2 bg-white border border-gray-300 rounded-full hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-teal-300 focus:ring-opacity-50"
            >
              <img src={service.category.iconUrl} alt={service.name} className="w-6 h-6 mr-2" />
